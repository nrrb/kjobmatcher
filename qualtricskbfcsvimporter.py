...
class QualtricsKBFCSVImporter(object):
    ...
    def __init__(self, filename=None, csv_content_string=None, legacy_format=False):
        ...
        self.data = dict()
        self.organization_rankings = list()
        ...
        for row in self.raw_data[self.__data_start_row_index:]:
            ...
            for organization_name, column_index in self.__organizations_by_column_index.items():
                            value = row[column_index] or self.__NONRESPONSE_VALUE_TEXT
                            if value == '0':
                                value = self.__NONRESPONSE_VALUE_TEXT
                            self.data[respondent_name][organization_name] = int(value)
        self.respondents = sorted(self.respondents)
        self.organizations = sorted(self.organizations)
        # Build the numeric 2-d array, with the rows corresponding to the respondents and the columns
        # corresponding to the organizations, with each numeric value representing the priority that
        # respondent gave to that organization (max value if the respondent did not specifically
        # rank it).
        for respondent in self.respondents:
            organization_rankings_row = list()
            for organization in self.organizations:
                organization_rankings_row.append(self.data[respondent][organization])
            self.organization_rankings.append(organization_rankings_row)

    def BestOrganizationFit(self):
        m = Munkres()
        indexes = m.compute(self.organization_rankings)
        bestfit_array = list()
        for row, column in indexes:
            respondent = self.respondents[row]
            organization = self.organizations[column]
            ranking = self.organization_rankings[row][column]
            bestfit_array.append({'respondent': respondent, 'organization': organization, 'ranking': ranking})
